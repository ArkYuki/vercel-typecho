...
function showThumb($obj, $link = false, $w, $h)
{
    ...
    $thumb = '';
    $attach = $obj->widget('Widget_Contents_Attachment_Related@' . $obj->cid . '-' . uniqid(), array('parentId'  => $obj->cid,'limit'     => 1,'offset'    => 0))->attachment;
	
	$name = md5($obj->cid);
   $file1 = ".".__TYPECHO_PLUGIN_DIR__."/Thumb/uploads/".$name.'.webp';
   $file2 = ".".__TYPECHO_PLUGIN_DIR__."/Thumb/uploads/".$name.'.jpg';
   if(file_exists($file1)){
        $thumb=Helper::options()->rootUrl.__TYPECHO_PLUGIN_DIR__."/Thumb/uploads/".$name.'.webp?'.filemtime($file1);
    }
   elseif(file_exists($file2)){
        $thumb=Helper::options()->rootUrl.__TYPECHO_PLUGIN_DIR__."/Thumb/uploads/".$name.'.jpg?'.filemtime($file2);
    }
